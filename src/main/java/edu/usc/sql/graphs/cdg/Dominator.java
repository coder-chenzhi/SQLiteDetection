...
//	private Set<DNode> allDNode = new HashSet<>();
	...
	private void initialize()
	{
		...
		for(NodeInterface n:allNode)
		{
			...
			else
			{
				...
				//	domSet.get(n).addAll(allNode);
				n.getDomSet().addAll(allNode);
			}
	...
	public boolean isDominate(NodeInterface source,NodeInterface dest)
	{
		...
		{
			if(n.equals(dest))
				if(n.getDomSet().contains(source))
					return true;
		}
		return false;
	}
		...
		while(change)
		{
			...
			for(NodeInterface n: dfsNodes)
			{
				...
				if(!n.getInEdges().isEmpty())
				{
					...
					for(NodeInterface nn: first.getDomSet())
						temp.add(nn);
					//intersect

					for(EdgeInterface e:n.getInEdges())
						temp.retainAll(e.getSource().getDomSet());
					//union itself
					if(!temp.contains(n))
						temp.add(n);
										
					
					if(!compareTwoSet(n.getDomSet(), temp))
					{
						change=true;
						n.getDomSet().clear();
	...
	public Map<NodeInterface,List<NodeInterface>> getDominatorTable()
	{
		...
		{
			for(NodeInterface n:dn.getDomSet())
				dTable.get(n).add(dn);
		}
		return dTable;
	}
	...
	public Map<NodeInterface,NodeInterface> computeImmediateDominator(){
		...
		for (NodeInterface n : allNode) {
			Set<NodeInterface> tempSet = new HashSet<NodeInterface>(n.getDomSet());
			tempSet.remove(n);
			tmp.put(n,tempSet );
		}
